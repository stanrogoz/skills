---
name: 5p-framework
description: Apply the 5P Framework (Pause, Play, Push, Polish, Present) by Stan Rogoz to turn a request into a decision-ready output instead of a quick first-pass answer. Use this whenever the user asks for a recommendation, a decision, presents a problem, a proposal, a strategy, an evaluation, or advice on "should we / which one / what's the best way" questions — especially when the output will be shown to stakeholders, a boss, a client, or a presentation. Also use it when the user explicitly mentions the 5P framework, "decision-ready", or asks you to stress-test or pressure-test a plan or idea.
---
...
Work through the five steps **in order**. The order matters: Pause, Play, Push, Polish, Present, framing before exploring, exploring before critiquing, critiquing before finalising. Skipping ahead recreates exactly the failure mode the loop exists to prevent.
...
- State the actual decision being made, a requirements and what "done" looks like.
...
If the decision itself is ambiguous — you genuinely can't tell what's being decided or for whom — always ask. Otherwise, state your framing explicitly and proceed; a visible framing the user can correct beats a stalled task.
...
- Aim for 3–5 real alternatives, including at least one unconventional or contrarian take, think outside-the-box.
...
- Surface hidden assumptions, counter-arguments, and failure modes. Don't be nice - critique.
- Verify claims where you can — check the numbers, the sources, the edge cases. If something can't be verified, flag it as an assumption rather than presenting it as fact.

### 4. Polish — finalise the output

Converge. Turn the surviving reasoning into the version you would actually ship proudly.

- Lead with the decision and the why — don't bury the recommendation.
...
- This is the most important step, work is never finished, until this is satisfied.
...
1. **Recommendation** — the decision and the core reason, up front.
...
Adapt the format to what Pause identified (a Slack reply needs less scaffolding than a board memo), but never drop the substance: alternatives considered, assumptions named, risks surfaced.
